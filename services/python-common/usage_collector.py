...
class UsageCollector:
    ...
    def stage_usage_summary(self) -> Dict[str, Dict[str, int]]:
        summary: Dict[str, Dict[str, int]] = {}
        for event in self._events:
            stage_id = NODE_TO_STAGE_ID.get(event.node_name)
            if not stage_id:
                continue
            ...
            stage_summary["prompt_tokens"] += event.prompt_tokens
            stage_summary["completion_tokens"] += event.completion_tokens
            stage_summary["total_tokens"] += event.total_tokens
            stage_summary["llm_calls"] += 1
            stage_summary["latency_ms"] += event.latency_ms
            if not event.success:
                stage_summary["failed_calls"] += 1
        return summary
