...
- 模块职责：
  ...
  - 分析任务提交、轮询、cancel/resume、export
  - 阶段化分析结果展示（优先 `stages`）
  - 高级分析配置：`selected_analysts`、research depth
...
- 模块职责：
  ...
  - usage 聚合与 admin 视图
  - 交易任务创建、查询、cancel/resume、统计、导出
  - Redis 任务协调与 runtime 对账
  - Feed board、偏好、like/save
...
- 当前状态：作为 `trading-service` / `market-data-service` 的共享 Python runtime
...
- 模块职责：
  - 持久化用户、email token、用户 API key、usage、feed、任务、决策
- 关键对象：
  - `User`
  - `UserAPIKey`
  - `EmailToken`
...
- 模块职责：
  - 本地编排与服务入口
  - backend / trading-service / market-data-service / openclaw-gateway / postgres / redis / nginx 协同启动
  - 部署方向预留
