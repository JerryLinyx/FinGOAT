...
- 用户注册、登录、JWT 鉴权、受保护接口可用（当前为基础账号模型）。
...
- 用户表仍是 `username + password` 的简化模型，尚未切到 email-first 的账号结构。
- email 登录尚未落地。
- 用户 Profile 页面与用户 API Key 配置页面尚未提供。
...
- 用户账号模型尚未完成 email-first 重构与迁移方案（当前仍偏 PoC 结构）。
...
- 用户侧能力尚未补齐：email 登录、Profile 自助编辑、API Key 配置入口缺失。
- Chart 页面缺少历史查询列表，常查代码复用效率低。
- TradingAgents 提交缺少“同代码+同日期”重复运行确认，容易出现无意重复执行。
...
- 为什么要做：账号体系是后续个性化配置和多端能力的根基，当前模型扩展性不足。
- 不足点：`username + password` 模型过于简化，缺少 email-first 语义和迁移策略。
- 解决真实问题：为身份体系标准化、账号找回、安全策略与用户画像扩展打基础。
- 设计路线：重设计用户表（email 唯一、password hash 语义、profile 字段预留）并实施兼容迁移。
- 备选路线：保持现有模型，仅局部打补丁。
- 选择原因：一次性完成基础模型治理，避免后续每次功能都受限于历史结构。

### 方向 J：用户自助配置能力（P1）

- 为什么要做：当前用户无法管理自己的资料和 provider 密钥，影响可用性和可控性。
- 不足点：缺少 Profile 页面与 API Key 配置入口，配置只能依赖服务端静态环境。
- 解决真实问题：支持用户自助管理身份信息与密钥配置，降低运维依赖并提升可扩展性。
- 设计路线：增加 Profile 与 API Key 页面，配套鉴权后端接口与敏感信息脱敏策略。
- 备选路线：继续使用运维侧统一密钥配置，不开放用户侧能力。
- 选择原因：v0.2 进入 MVP 阶段后，用户侧配置能力是产品化必要条件。

### 方向 K：Chart 查询历史体验增强（P1）

- 为什么要做：高频查同一批股票时，重复输入代码会拉低分析效率。
- 不足点：Chart 当前无历史查询入口，也没有最近查询排序。
- 解决真实问题：提升复查速度与可用性，降低重复输入成本。
- 设计路线：增加历史夹/历史列表，按代码去重，重复查询时置顶。
- 备选路线：不做历史记录，仅保留手动输入。
- 选择原因：实现成本低、收益直接，且与图表高频使用场景匹配。

### 方向 L：Trading 分析重复运行防护（P1）

- 为什么要做：重复提交同代码同日期分析会增加模型与数据调用成本，也干扰任务列表可读性。
- 不足点：当前提交流程没有重复检测后的用户确认环节。
- 解决真实问题：降低误触发重复分析概率，同时保留用户主动重跑的权利。
- 设计路线：按“同用户+同代码+同日期”判重，命中后弹窗二次确认，确认才发起任务。
- 备选路线：完全禁止重复提交，或仅前端本地提醒。
- 选择原因：二次确认在成本控制与用户灵活性之间更平衡。
...
- 目标：完成用户模型升级与 email 登录闭环。
- 产出：用户表迁移方案、email 登录注册接口、兼容迁移策略与回滚预案。
- 依赖：阶段 1 的核心任务链路稳定。
- 风险：历史账号兼容与认证回归风险。
...
- 依赖：阶段 1 的 schema 稳定，阶段 2 的认证契约稳定。
- 风险：历史配置兼容处理。

### 阶段 4：前端结构重整（P1）

- 目标：降低前端维护复杂度，支撑持续迭代。
- 产出：状态域拆分、测试补强、页面行为一致性校验、Profile/API Key 页面落地、Trading 去重确认交互落地。
- 依赖：阶段 1-3 的接口语义稳定。
- 风险：迁移期间 UI 回归。
