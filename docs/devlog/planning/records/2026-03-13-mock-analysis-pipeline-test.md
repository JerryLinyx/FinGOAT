...
- Extract single-payload worker handling into `process_analysis_payload()` inside `langchain-v1/trading_service.py`.
- Add a dedicated test folder at `langchain-v1/tests/mock_pipeline/`.
...
```bash
cd /Users/linyuxuan/workSpace/FinGOAT/langchain-v1
